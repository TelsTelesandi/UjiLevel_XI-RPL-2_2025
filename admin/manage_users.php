            ...
            case 'edit':
                ...
                if ($user_id != $_SESSION['user_id']) {
                    try {
                        ...
                        // Ambil semua event_id yang dimiliki user
                        $query = "SELECT event_id FROM event_pengajuan WHERE user_id = :user_id";
                        $stmt = $conn->prepare($query);
                        $stmt->bindParam(':user_id', $user_id);
                        $stmt->execute();
                        $event_ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

                        // Jika user memiliki event
                        if (!empty($event_ids)) {
                            // Ubah array jadi string comma-separated untuk query IN (...)
                            $in_clause = implode(',', array_fill(0, count($event_ids), '?'));

                            // Hapus data verifikasi_event terlebih dahulu
                            $query = "DELETE FROM verifikasi_event WHERE event_id IN ($in_clause)";
                            $stmt = $conn->prepare($query);
                            $stmt->execute($event_ids);

                            // Hapus event_pengajuan milik user
                            $query = "DELETE FROM event_pengajuan WHERE event_id IN ($in_clause)";
                            $stmt = $conn->prepare($query);
                            $stmt->execute($event_ids);
                        }
                        ...
                        $_SESSION['message'] = 'User beserta data terkait berhasil dihapus!';
                        $_SESSION['message_type'] = 'success';
