<?php
// Pastikan user sudah login
if (!isset($_SESSION['user_id'])) {
    header("Location: index.php?action=login");
    exit();
}

// Ambil detail event
$eventId = $_GET['id'] ?? 0;
$event = $queries->getEventById($eventId);

// Validasi kepemilikan event
if (!$event || $event['user_id'] != $_SESSION['user_id']) {
    header("Location: index.php?action=my_events");
    exit();
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Detail Event - Event Management System</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <style>
        body {
            background: linear-gradient(135deg, #1e40af 0%, #3730a3 100%);
            min-height: 100vh;
        }
        .dashboard-card {
            background: rgba(255, 255, 255, 0.1);
            backdrop-filter: blur(10px);
            border: 1px solid rgba(255, 255, 255, 0.2);
        }
    </style>
</head>
<body>
    <div class="flex">
        <!-- Sidebar -->
        <div class="w-64 bg-indigo-900 min-h-screen p-4">
            <div class="flex items-center space-x-4 mb-6">
                <i class="fas fa-user text-white text-2xl"></i>
                <h1 class="text-white text-xl font-bold">User Panel</h1>
            </div>
            
            <nav class="space-y-2">
                <a href="index.php?action=dashboard" 
                   class="flex items-center space-x-2 text-gray-300 hover:bg-indigo-800 p-2 rounded-lg">
                    <i class="fas fa-tachometer-alt"></i>
                    <span>Dashboard</span>
                </a>
                <a href="index.php?action=submit_event" 
                   class="flex items-center space-x-2 text-gray-300 hover:bg-indigo-800 p-2 rounded-lg">
                    <i class="fas fa-plus-circle"></i>
                    <span>Ajukan Event</span>
                </a>
                <a href="index.php?action=my_events" 
                   class="flex items-center space-x-2 text-gray-300 hover:bg-indigo-800 p-2 rounded-lg">
                    <i class="fas fa-calendar-alt"></i>
                    <span>Event Saya</span>
                </a>
            </nav>
        </div>

        <!-- Main Content -->
        <div class="flex-1">
            <!-- Top Bar -->
            <div class="bg-white bg-opacity-10 p-4 flex justify-between items-center">
                <h2 class="text-2xl font-bold text-white">Detail Event</h2>
                <div class="flex items-center space-x-4">
                    <div class="text-white">
                        <span class="mr-2">Welcome,</span>
                        <span class="font-semibold"><?php echo htmlspecialchars($_SESSION['username']); ?></span>
                    </div>
                    <a href="index.php?action=logout" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">
                        <i class="fas fa-sign-out-alt mr-2"></i>Logout
                    </a>
                </div>
            </div>

            <!-- Main Content Area -->
            <div class="p-8">
                <div class="dashboard-card rounded-xl p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h3 class="text-lg font-semibold text-white mb-4">Informasi Event</h3>
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Judul Event</label>
                                    <p class="mt-1 text-white"><?php echo htmlspecialchars($event['judul_event']); ?></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Jenis Kegiatan</label>
                                    <p class="mt-1 text-white"><?php echo htmlspecialchars($event['jenis_kegiatan']); ?></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Total Pembiayaan</label>
                                    <p class="mt-1 text-white">Rp <?php echo number_format($event['total_pembiayaan'], 0, ',', '.'); ?></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Status</label>
                                    <span class="mt-1 inline-block px-3 py-1 text-sm rounded-full 
                                        <?php 
                                        echo match($event['status']) {
                                            'menunggu' => 'bg-yellow-500',
                                            'disetujui' => 'bg-green-500',
                                            'ditolak' => 'bg-red-500',
                                            'selesai' => 'bg-gray-500',
                                            default => 'bg-gray-500'
                                        };
                                        ?>">
                                        <?php echo htmlspecialchars($event['status']); ?>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-white mb-4">Detail Waktu</h3>
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Tanggal Pengajuan</label>
                                    <p class="mt-1 text-white"><?php echo date('d F Y H:i', strtotime($event['tanggal_pengajuan'])); ?></p>
                                </div>
                                <?php if (isset($event['tanggal_update']) && $event['tanggal_update']): ?>
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Terakhir Diupdate</label>
                                    <p class="mt-1 text-white"><?php echo date('d F Y H:i', strtotime($event['tanggal_update'])); ?></p>
                                </div>
                                <?php endif; ?>
                                <?php if (isset($event['tanggal_selesai']) && $event['tanggal_selesai']): ?>
                                <div>
                                    <label class="block text-sm font-medium text-gray-300">Tanggal Selesai</label>
                                    <p class="mt-1 text-white"><?php echo date('d F Y H:i', strtotime($event['tanggal_selesai'])); ?></p>
                                </div>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="md:col-span-2">
                            <h3 class="text-lg font-semibold text-white mb-4">Deskripsi Event</h3>
                            <p class="text-gray-300 whitespace-pre-line"><?php echo nl2br(htmlspecialchars($event['deskripsi'])); ?></p>
                        </div>
                        <div class="md:col-span-2">
                            <h3 class="text-lg font-semibold text-white mb-4">File Proposal</h3>
                            <?php if (isset($event['file_proposal']) && $event['file_proposal'] && file_exists($event['file_proposal'])): ?>
                                <a href="<?php echo htmlspecialchars($event['file_proposal']); ?>" target="_blank" 
                                   class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                                    <i class="fas fa-download mr-2"></i>
                                    Download Proposal
                                </a>
                            <?php else: ?>
                                <p class="text-gray-300">File proposal tidak tersedia</p>
                            <?php endif; ?>
                        </div>
                    </div>

                    <div class="mt-8 flex justify-end space-x-4">
                        <a href="javascript:history.back()" 
                           class="px-4 py-2 bg-white/10 text-white rounded-lg hover:bg-white/20 transition-colors">
                            Kembali
                        </a>
                        <?php if ($event['status'] === 'disetujui'): ?>
                        <button onclick="closeEvent(<?php echo $event['event_id']; ?>)"
                                class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                            <i class="fas fa-check-circle mr-2"></i>
                            Selesaikan Event
                        </button>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <script>
    function closeEvent(eventId) {
        if (confirm('Apakah Anda yakin ingin menyelesaikan event ini? Event yang sudah selesai tidak dapat diubah kembali.')) {
            fetch('index.php?action=close_event', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: 'id=' + eventId
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    alert('Event berhasil diselesaikan');
                    location.reload();
                } else {
                    alert('Gagal menyelesaikan event: ' + data.message);
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('Terjadi kesalahan saat menyelesaikan event');
            });
        }
    }
    </script>
</body>
</html> 