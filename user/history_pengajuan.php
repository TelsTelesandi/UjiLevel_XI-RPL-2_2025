...
// Get user's events with status history
$query = "SELECT ep.*, 
          CASE 
              WHEN ep.status = 'approved' THEN 'Disetujui'
              WHEN ep.status = 'rejected' THEN 'Ditolak'
              ELSE 'Menunggu Persetujuan'
          END as status_text,
          DATE_FORMAT(ep.created_at, '%d %M %Y %H:%i') as tanggal_pengajuan
...
<html>
<head>
    <title>History Pengajuan - Aplikasi Ekskul</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="welcome-section">
            ...
            <p>Lihat riwayat pengajuan event ekskul Anda.</p>
        </div>
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td><span class='status-badge status-" . htmlspecialchars($row['status']) . "'>" . 
                                 htmlspecialchars($row['status_text']) . "</span></td>";
                            echo "<td>";
                        ...
                        echo "<tr><td colspan='6' style='text-align: center;'>Belum ada history pengajuan</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
