...
// Get event details
$query = "SELECT * FROM event_pengajuan WHERE event_id = ? AND user_id = ? AND status = 'pending'";
...
if (mysqli_num_rows($result) === 0) {
    $_SESSION['error'] = "Event tidak ditemukan atau tidak dapat diedit.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $file_path = $event['file_path']; // Keep existing file by default
            ...
            if ($file['size'] > 5 * 1024 * 1024) {
                ...
                if (move_uploaded_file($file['tmp_name'], $target_path)) {
                    ...
                    if (!empty($event['file_path'])) {
                        $old_file = $upload_dir . '/' . $event['file_path'];
    ...
    if (empty($error)) {
        // Update event
        $query = "UPDATE event_pengajuan SET nama_event = ?, deskripsi = ?, tanggal = ?, file_path = ? WHERE event_id = ? AND user_id = ?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "ssssii", $nama_event, $deskripsi, $tanggal, $file_path, $event_id, $_SESSION['user_id']);
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="date" class="form-control" id="tanggal" name="tanggal" 
                       value="<?php echo htmlspecialchars($event['tanggal']); ?>" required>
            </div>

            <div class="form-group">
                <label for="deskripsi">Deskripsi</label>
                <textarea class="form-control" id="deskripsi" name="deskripsi" required><?php 
                    echo htmlspecialchars($event['deskripsi']); 
                ?></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if (!empty($event['file_path'])): ?>
                    <div class="current-file">
                        File saat ini: 
                        <a href="../uploads/<?php echo htmlspecialchars($event['file_path']); ?>" target="_blank">
                            <?php echo htmlspecialchars(basename($event['file_path'])); ?>
                        </a>
                    </div>
                <?php endif; ?>
